...
public final class ContextCoordinator: RecentActivityContext {
    ...
    public func getRecentActivityContext() async -> String {
        await getRecentActivityContext(hint: nil)
    }
    ...
    public func capture(trigger: ContextCaptureTrigger, cursorLocation: CGPoint?, bypassPause: Bool = false) async {
        ...
        do {
            ...
            case .unchanged:
                ...
                NSLog("[ContextCoordinator] Captured \(trigger.rawValue) context for \(metadata.appName) — classified UNCHANGED, skipping Gemini fan-out.")
            case .minorChange:
                if let previousObservation = await store.lastReducerObservation() {
                    ...
                    NSLog("[ContextCoordinator] Captured \(trigger.rawValue) context for \(metadata.appName) — classified MINOR_CHANGE, dispatching update lane only.")
                } else {
                    // No previous observation to update — fall through to full
                    // fan-out so we get a base observation to update next time.
                    ...
                    NSLog("[ContextCoordinator] Captured \(trigger.rawValue) context for \(metadata.appName) — minor change without baseline observation, running full fan-out.")
                }
            case .majorChange:
                scheduleGeminiObservation(
                    for: contextSnapshot,
                ...
                NSLog("[ContextCoordinator] Captured \(trigger.rawValue) context for \(metadata.appName) — classified MAJOR_CHANGE, running full fan-out.")
            }

            await dirtyThresholdsTracker.observe(classification.classification)
            NSLog("[ContextCoordinator] Captured \(trigger.rawValue) context for \(metadata.appName) with \(recognizedText.count) OCR text items (\(classification.classification.label), hamming=\(classification.comparison?.hammingDistance ?? -1), area=\(String(format: "%.3f", classification.comparison?.changedAreaFraction ?? -1)))")
        } catch {
    ...
    private func recordUnchangedEvent(
        ...
        await aiObservationLog.record(ContextAIObservationEvent(
            ...
            dirtyClassification: ContextDirtyClassification.unchanged.label,
            dirtyHammingDistance: comparison?.hammingDistance,
            dirtyChangedAreaFraction: comparison?.changedAreaFraction
        ))
    }
        ...
        Task(priority: .utility) { [snapshot, imageData, previousSnapshot, input, lanes, fanoutClassification] in
            ...
            case .skip(let reason):
                await aiObservationLog.record(ContextAIObservationEvent(
                    ...
                    dirtyClassification: (comparison?.classification ?? .majorChange).label,
                    dirtyHammingDistance: comparison?.hammingDistance,
                    dirtyChangedAreaFraction: comparison?.changedAreaFraction
                ))
            ...
            if !laneResults.isEmpty {
                ...
                do {
                    ...
                    NSLog("[ContextCoordinator] Gemini LLM reducer succeeded in \(reducerElapsedMs)ms")
                } catch {
                    let reducerElapsedMs = Int(Date().timeIntervalSince(reducerStartedAt) * 1000)
                    NSLog("[ContextCoordinator] Gemini LLM reducer failed in \(reducerElapsedMs)ms: \(error) — falling back to Swift reducer")
                }
            }
            if reducerObservation == nil {
                ...
                if reducerObservation != nil {
                    NSLog("[ContextCoordinator] Used Swift fallback reducer for \(laneResults.count) lanes")
                }
            }
