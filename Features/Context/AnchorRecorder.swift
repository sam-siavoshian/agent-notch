...
/// Observes EventLog for repeated key/click sequences and promotes them to L3
/// recipes (per-app coord-free shortcuts) at 3 matching occurrences. Persists
/// per-app recipe collections under
/// ~/Library/Application Support/AgentNotch/ContextMemory/anchors/<bundleID>.json.
///
/// Pipeline: poll EventLog every 5s → segment by boundary events
/// (screen/dwell/appSwitch) → normalize typed text into template slots →
/// match by (app + signature) → promote candidate at seenCount == 3.
public final class AnchorRecorder {
    ...
    private static let promotionThreshold: Int = 3
    ...
    private static func isTriggerEvent(_ event: CEvent) -> Bool {
        ...
        case let .input(_, _, _, submitKey, modifiers):
            // SubmitKey "shortcut" is set by KeystrokeMonitor for cmd/ctrl/alt
            // keypresses. Non-shift modifier counts too; bare shift is just casing.
    ...
    private func flushPending() {
        ...
        guard let bundleID = pendingApp, !bundleID.isEmpty, pendingHasTrigger else { return }
        ...
        // shortcuts tally immediately — single keypresses are inherently generalizable.
        for step in steps {
        ...
        guard steps.count >= 2 else { return }   // singletons aren't multi-step recipes
        ...
        // (3) Auto-promote on first observation when all type-step values are
        // already generalized (wrapped in <...>): no literal user text to abstract.
        ...
        if isFullyGeneralized {
            collection.recipes.append(CRecipe(
        ...
        // (4) Multi-step with literal text — 3-occurrence candidate→promote flow.
        if let idx = collection.candidates.firstIndex(where: { Self.signature(of: $0.steps) == signature }) {
            ...
            if collection.candidates[idx].seenCount >= Self.promotionThreshold {
                var promoted = collection.candidates.remove(at: idx)
                promoted.confidence = Double(promoted.seenCount) / Double(promoted.seenCount + 2)
                collection.recipes.append(promoted)
                AgentObservabilityLog.shared.record(.memoryMutation(
                    id: UUID(), t: Date(),
                    kind: .recipePromoted,
        ...
        collection.candidates.append(CRecipe(
    ...
    /// ends with `\r` (or has submitKey "return"), capture it as `.shellCmd`
    /// rather than `.type`.
    private static func terminalAwareStepFor(_ event: CEvent, bundleID: String) -> CRecipe.Step? {
        ...
        // First token must look command-ish (short, alpha-ish).
    ...
    private static func stepFor(_ event: CEvent) -> CRecipe.Step? {
        ...
        case let .click(elementLabel, _, _):
            // No coord-free click step kind yet — encode as a typed sentinel
            // the Selector can ax_press by.
    ...
    /// Deterministic slot typing: replace literal text with a template slot when
    /// it looks like a person/file/url/query, otherwise keep literal.
